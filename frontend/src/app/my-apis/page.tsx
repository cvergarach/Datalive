...
export default function APIsPage() {
    ...
    const fetchApis = async (projectId: string) => {
        setLoading(true);
        try {
            const response = await api.get(`/projects/${projectId}/apis`);
                                            ...
                                            <div className="flex items-start justify-between">
                                                ...
                                                <div className="flex gap-2">
                                                    ...
                                                    <Button
                                                        variant="ghost"
                                                        size="sm"
                                                        onClick={() => setExpandedApi(expandedApi === apiItem.id ? null : apiItem.id)}
                                                    >
                                                        {apiItem._count?.api_endpoints || 0} Endpoints
                                                ...
                                                <div className="overflow-x-auto">
                                                    <table className="w-full text-left text-sm">
                                                        ...
                                                        <tbody className="divide-y divide-gray-100">
                                                            {/* We should ideally fetch specific endpoints if not included in list response */}
                                                            {/* For now, assuming they might be side-loaded or we need another fetch */}
                                                            <tr className="bg-transparent hover:bg-white transition-colors">
                                                                <td colspan={4} className="px-6 py-10 text-center text-gray-400 italic">
                                                                    Loading detailed endpoints list...
                                                                </td>
                                                            </tr>
                                                        </tbody>
                                                    </table>
